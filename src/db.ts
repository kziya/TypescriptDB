...
import {IDbTable, IDbTableDataType} from "./types/db.types";


export class DbTable<TSchema extends object, TUpdateSchema extends object> implements IDbTable<TSchema, TUpdateSchema>{
    ...
    constructor(dbFilePath:  string) {
        ...
        fs.readFile(this.dbFilePath, (err, data) => {
            if (err) {
                ...
                fs.writeFile(this.dbFilePath, this.toJson(this.data), () => { console.log('screated') });
            } else {
                try {
                    this.data = this.toJS(data.toString());
                } catch (e) {
                    // catch parse error
                    fs.writeFile(this.dbFilePath, this.toJson(this.data), () => console.log('created'));
                }
            }
        });
    }
    ...
    add(dataObject: TSchema){
        const curId = this.data[this.data.length - 1]?._id ? this.data[this.data.length - 1]._id + 1: 1;
    ...
    private checkElem(elem: TSchema, props: TUpdateSchema | TSchema | IDbTableDataType){
